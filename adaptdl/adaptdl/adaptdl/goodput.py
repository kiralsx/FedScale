...
class GoodputFunction(object):
    ...
    def evaluate(self, num_nodes, num_replicas, atomic_bsz, accum_steps):
        ...
        # print(f"throughput: {self.throughput(num_nodes, num_replicas, atomic_bsz,accum_steps).flatten()}")
        # print(f"efficiency: {self.efficiency(batch_size).flatten()}")
    ...
    def efficiency(self, batch_size):
        ...
        # print(f"grad_sqr: {grad_sqr}, grad_var: {grad_var}, scale: {scale}, denom: {denom}")
    ...
    def optimize(self, num_nodes, num_replicas, max_batch_size=None,
        ...
        # print(f"min_atomic_bsz: {min_atomic_bsz} max_atomic_bsz: {max_atomic_bsz}")
        ...
        # print(f"output shape: {output_shape} output_scalar: {output_scalar}")
        ...
        # print(f"local_bsz:{local_bsz.flatten()}")
        eps = 1e-8  # Tolerance for floor/ceil operations.
        if accumulation:
            return NotImplementedError("accumulation is set!!!")
            ...
            # Note: modify the original code, scale bsz even for only 1 gpu
            atomic_bsz = np.ceil(local_bsz - eps).astype(int)
            # print(f"atomic_bsz: {atomic_bsz.flatten()}")
