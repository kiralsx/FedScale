...
class AdaptiveEpoch():
    ...
    def finished_epochs(self):
        return self._state.finished_epochs
    
    def remaining_epochs_until(self, epoch):
        """
        Iterate over epochs in a way that is consistent with checkpoint-restarts.
        For example:
        ...
        if self.current_epoch() is not None:
            raise RuntimeError("overlapping epoch loops detected")
        if self.finished_epochs() < epoch:
            LOG.info("starting at epoch %s", self.finished_epochs())
        else:
            LOG.info("skipping all epochs up to %s", epoch)
        while self.finished_epochs() < epoch:
            self._state.current_epoch = self.finished_epochs()
            try:
                yield self.current_epoch()
            finally:
                # Try to catch any exits from epoch loop, including breaks and
                # Exceptions. See https://www.peterbe.com/plog/generatorexit.
                self._state.finished_epochs += 1
                self._state.current_epoch = None