...
#         # print(f"replica_rank(): {adaptdl.env.replica_rank()}, time.time() - _PREV_REPORT: {time.time() - _PREV_REPORT}")
...
class MetricsState(adaptdl.checkpoint.State):
    def __init__(self, ckpt_dir):
        ...
        adaptdl.checkpoint.load_state(self, ckpt_dir=ckpt_dir)

        self._PREV_REPORT = None
        self._GRAD_PARAM_DICT = {}
    ...
    def profile_step_start(self, atomic_bsz):
        state = self._metrics_state()
        state.atomic_bsz = atomic_bsz
        state.step_start = time.time()
        state.sync_time = 0.0

    def profile_sync_time(self, sync_time):
        self._metrics_state().sync_time += sync_time

    
    def profile_step_commit(self, accumulation_step=False):
        state = self._metrics_state()
        step_time = time.time() - state.step_start
        num_nodes = adaptdl.env.num_nodes()
        num_replicas = adaptdl.env.num_replicas()
        ...
        if not accumulation_step:
            if self._PREV_REPORT is None:
                self._PREV_REPORT = time.time()
            # print(f"replica_rank(): {adaptdl.env.replica_rank()}, time.time() - _PREV_REPORT: {time.time() - _PREV_REPORT}")
            if adaptdl.env.replica_rank() == 0 and time.time() - self._PREV_REPORT > 15:
                self._fit_perf_params()
                self._report_sched_hints()
                self._PREV_REPORT = time.time()

    
    def update_grad_params(self, edp_key, grad_norm_sqr, grad_variance):
        # print("!!!!!Grad params get updated!!")
        self._GRAD_PARAM_DICT[edp_key] = np.asarray([grad_norm_sqr, grad_variance])
        grad_params = sum(self._GRAD_PARAM_DICT.values())
        self._metrics_state().grad_params = (grad_params[0], grad_params[1])


    
    def update_progress(self, progress):
        self._metrics_state().progress = progress


    def get_progress(self):
        return self._metrics_state().progress
    

    def set_batch_size(self, init_batch_size, max_batch_size, local_bsz_bounds,
                   gradient_accumulation):
        state = self._metrics_state()
        state.init_batch_size = init_batch_size
        state.max_batch_size = max_batch_size
