...
def train(args, model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % args.log_interval == 0:
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tbsz:{}\tvar:{}\tsqr:{}\tLoss: {:.6f}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader), train_loader.current_batch_size, model.gns.var_avg(), model.gns.sqr_avg(),loss.item()))
            if args.dry_run:
                break
...
def main():
    ...
    train_loader.autoscale_batch_size(1028, local_bsz_bounds=(32, 1028)) # Changed in step 3, optional
